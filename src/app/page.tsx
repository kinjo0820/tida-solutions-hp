    ...
    <div className="min-h-screen bg-[#050505] text-slate-50 font-sans selection:bg-[var(--accent)/30]">
      ...
      <section className="relative pt-32 pb-24 px-6 max-w-7xl mx-auto text-center">
        ...
        <div className="flex flex-col md:flex-row justify-center gap-4">
          ...
          <button className="bg-white/5 border border-white/10 text-white px-10 py-4 rounded-full font-bold text-lg hover:bg-[var(--accent)] hover:text-black transition-all">
            受託開発を相談する
          </button>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-2 gap-6">
            {[
              ...
              <div key={i} className="group p-10 rounded-3xl bg-white/2 border border-white/5 hover:border-(--accent) transition-all">
                <div className="w-14 h-14 bg-white/5 text-(--accent) rounded-2xl flex items-center justify-center mb-8 group-hover:bg-(--accent) group-hover:text-white transition-all">
                  {item.icon}
                </div>
      ...
      {/* Domains */}
      <section id="projects" className="py-32 px-6 bg-linear-to-b from-transparent to-orange-950/20">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-4xl font-bold mb-16">Focus Domains</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {["不動産", "飲食", "観光", "スポーツ", "小売", "教育", "店舗", "テーマパーク"].map((name) => (
              <div key={name} className="py-8 rounded-2xl border border-white/5 bg-white/1 hover:bg-white/3 transition-all cursor-default group">
                <span className="text-slate-400 group-hover:text-(--accent) transition-colors">{name}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 会社概要 */}
      <section id="company" className="py-20 px-6 bg-transparent border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-2xl font-bold mb-6 text-white">会社概要</h2>
          <div className="grid md:grid-cols-2 gap-6 text-slate-300">
            <div>
              <dl className="space-y-3">
                <div>
                  <dt className="text-sm text-slate-400">会社名</dt>
                  <dd className="font-medium text-white">株式会社Tida Solutions（株式会社ティーダソリューションズ）</dd>
                </div>
                <div>
            ...
            <div>
              ...
              <p className="mt-2 text-slate-300">メール: <a href="mailto:taichi@tida-intern.com" className="text-(--accent) font-medium">taichi@tida-intern.com</a></p>
              <p className="mt-2 text-slate-300">営業時間: 平日 10:00〜19:00</p>
              <p className="mt-4 text-slate-400">Google マップ: <a target="_blank" rel="noreferrer" href="https://www.google.com/maps/search/?api=1&query=%EF%BC%91904-0004%20%E6%B2%96%E7%B8%84%E7%9C%8C%E6%B2%96%E7%B8%84%E5%B8%82%E4%B8%AD%E5%A4%AE1%E4%B8%81%E7%9B%AE7-8%202%E9%9A%8E%20KSA%20Works" className="text-(--accent) font-medium">地図で開く</a></p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="py-20 px-6 border-t border-white/5 text-center">
        <p className="text-slate-400 text-sm mb-4">© 2026 Tida Solutions Inc.</p>
        <p className="text-slate-500 text-xs font-mono">// Accelerating Human Progress with AI-Native Solutions</p>
      </footer>
    </div>
  );
}